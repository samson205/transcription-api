...
def extract_operator_embedding_task(operator_id: int, file_path: str):
    logger.info("operator_id=%s Starting voice embedding extraction file=%s", operator_id, file_path)
    try:
        ...
        logger.exception("operator_id=%s Failed extracting voice embedding file=%s", operator_id, file_path)
        raise
    finally:
